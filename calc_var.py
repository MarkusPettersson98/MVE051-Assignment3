...
def standard_deviation(mean, text):
	variance = calcVar(mean, text)
	return variance**(1/2)
...
print("English redroom", [np.around(var_english - (1.6449 * stdev_english)/(len(redroom)**(1/2)),5), np.around(var_english + (1.6449 * stdev_english)/(len(redroom)**(1/2)),5)])
print("Swedish rödarummet", [np.around(var_swedish - (1.6449 * stdev_swedish)/(len(rodarummet)**(1/2)),5), np.around(var_swedish + (1.6449 * stdev_swedish)/(len(rodarummet)**(1/2)),5)])
